...
- **Public repo `Em3ritus/simulacra`.** Commit identity = `Em3ritus <152051018+Em3ritus@users.noreply.github.com>` (already the repo's local git config). Never commit the OS username, absolute `C:\Users\<name>` paths, real MACs/SSIDs, or `private/` / `.superpowers/` scratch.
